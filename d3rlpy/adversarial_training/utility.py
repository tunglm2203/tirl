...
class EvalLogger():
    ...
    def write_header(self):
        ...
        if 'transform_params' in self.exp_params:
            ...
            attack_type = self.exp_params['transform_params'].get('attack_type', None)
            if attack_type is None:
                attack_type = self.exp_params['transform_params'].get('attack_type_actor', None)
            ...
            robust_type = self.exp_params['transform_params'].get('robust_type', None)
            if robust_type is None:
                robust_type = self.exp_params['transform_params'].get('robust_type_actor', None)
            self.writer.write("Robust type: %s\n" % (robust_type))

            critic_reg_coef = self.exp_params['transform_params'].get('critic_reg_coef', None)
            if critic_reg_coef is None:
                critic_reg_coef = self.exp_params['transform_params'].get('q_reg_coef', None)

            actor_reg_coef = self.exp_params['transform_params'].get('actor_reg_coef', None)
            if actor_reg_coef is None:
                actor_reg_coef = self.exp_params['transform_params'].get('pi_reg_coef', None)
