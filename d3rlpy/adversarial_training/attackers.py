...
def critic_normal_attack(x, _policy, _q_func, denoiser, epsilon, num_steps, step_size,
                         _obs_min_norm, _obs_max_norm,
                         q_func_id=0, optimizer='pgd', clip=True, use_assert=True, whitebox=True):
    """" NOTE: x must be normalized """""
    ...
    # gray = True
    if gray:
        _policy.use_input_transformation = False
    ...
    for _ in range(num_steps):
        ...
        if denoiser is not None and whitebox:
            denoised_x = denoiser.denoise(adv_x)
            adv_x = denoised_x
    ...
    if gray:
        _policy.use_input_transformation = True
...
def actor_state_attack_mean(x, _policy, _q_func, denoiser, epsilon, num_steps, step_size, _obs_min_norm, _obs_max_norm,
