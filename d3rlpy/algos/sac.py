...
from ..models.torch.vector_quantization import TRANSFORMS_REQUIRED_UPDATE
...
class SAC(AlgoBase):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self._use_input_transformation = use_input_transformation
        self._transformation_type = transformation_type
        self._number_embeddings = number_embeddings
        self._embedding_dim = embedding_dim
        self._decay = decay
        self._vq_loss_weight = vq_loss_weight
        self._vq_reduction = vq_reduction
        self._bdr_step = bdr_step
    ...
    def _create_impl(
        ...
        policy_args = dict(
            use_input_transformation=self._use_input_transformation,
            transformation_type=self._transformation_type,
            number_embeddings=self._number_embeddings,
            embedding_dim=self._embedding_dim,
            decay=self._decay,
            vq_reduction=self._vq_reduction,
            bdr_step=self._bdr_step
        )
        self._impl.build(policy_args)

        if self._use_input_transformation and self._transformation_type in TRANSFORMS_REQUIRED_UPDATE:
    ...
    def _update(self, batch: TransitionMiniBatch) -> Dict[str, float]:
        ...
        if self._use_input_transformation:
            if self._transformation_type in ["ema", "sgd", "vanilla_sgd"]:
