...
from ...models.torch.vector_quantization import TRANSFORMS_REQUIRED_UPDATE, TRANSFORMS_REQUIRED_LOSS
...
class SACImpl(DDPGBaseImpl):
    ...
    def _build_actor(self, use_input_transformation: bool = False, transformation_type: str = "ema",
                     number_embeddings: int = 128, embedding_dim: int = 1, decay: float = 0.99,
                     vq_reduction: bool = True,
                     bdr_step: float = 0.1) -> None:
        self._policy = create_squashed_normal_policy(
            self._observation_shape,
            self._action_size,
            self._actor_encoder_factory,
            use_input_transformation=use_input_transformation, transformation_type=transformation_type,
            number_embeddings=number_embeddings, embedding_dim=embedding_dim, decay=decay, vq_reduction=vq_reduction,
            bdr_step=bdr_step
        )
    ...
    def update_actor(self, batch: TorchMiniBatch) -> Dict:
        ...
        if self._loss_type == "normal":
            ...
            if self.policy.use_input_transformation and self.policy.transformation_type in TRANSFORMS_REQUIRED_UPDATE and self.allow_update_codebook:
                self.policy.vq_input.enable_update_codebook()

            loss, actor_q_loss, entropy, extra_outs = self.compute_actor_loss(batch)

            if self.policy.use_input_transformation and self.policy.transformation_type in TRANSFORMS_REQUIRED_UPDATE:
            ...
            if self.policy.transformation_type in TRANSFORMS_REQUIRED_LOSS and \
                ((len(vq_loss.shape) == 0 and vq_loss != -1.0 ) or (len(vq_loss.shape) > 0)):
                if self._autoscale_vq_loss:
                    if self.policy.vq_input.reduction:
                        scale = (actor_q_loss.abs().mean().detach() / self._scale_factor).detach()
                    else:
                        scale = (actor_q_loss.abs().detach() / self._scale_factor).detach()
                    loss += (scale * vq_loss).mean()
                else:
                    loss += self.vq_loss_weight * vq_loss

            loss.backward()
            self._actor_optim.step()
            if len(vq_loss.shape) > 0:
            ...
            if self.policy.use_input_transformation and self.allow_update_codebook:
                self.policy.vq_input.enable_update_codebook()
            loss, actor_q_loss, entropy, tanh_dist, extra_outs = self.compute_actor_loss_for_at(batch)
            if self.policy.use_input_transformation:
                self.policy.vq_input.disable_update_codebook()
        ...
        if self.policy.use_input_transformation and self.policy.transformation_type in ["ema", "sgd", "vanilla_sgd"]:
            summary_logs.update({
