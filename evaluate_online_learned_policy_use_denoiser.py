...
def eval_func(algo, denoiser, env, writer, attack_type, attack_epsilon, params):
    ...
    if multiprocessing:
        ...
        if not _args.disable_clean:
            unorm_score = eval_multiprocess_wrapper(algo, denoiser, eval_clean_env, env_list, _args)
...
def main(args):
    args.eval_logdir = f"./evaluation/evaluation_online/clean_bb/{ENV_NAME_MAPPING[args.dataset]}"
    ...
    writer.print("Average clean: mean=%.2f, std=%.2f, median=%.2f, iqm=%.2f, og=%.2f (%d seeds)\n" %
                 (MEAN([unorm_score_clean[0]]), np.std(unorm_score_clean, axis=1).squeeze(),
                  MEDIAN([unorm_score_clean[0]]), IQM([unorm_score_clean[0]]), OG([unorm_score_clean[0]]),
                  n_seeds))
