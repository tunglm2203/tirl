...
from d3rlpy.models.torch.vector_quantization import AEDenoiser, VAEDenoiser
import torch
...
SUPPORTED_ATTACKS = ['none', 'random', 'minQ', 'minQ_rand', 'actor_mad', 'actor_mad_rand', 'sarsa', 'vq_attack']
...
parser.add_argument('--whitebox', action='store_true', default=False)
...
parser.add_argument('--transformation_type', type=str, default="ema", choices=["ema", "sgd", "vanilla_sgd", "bdr"])
...
parser.add_argument('--use_vaed', action='store_true', default=False)
parser.add_argument('--eps_aug', type=float, default=0.05)
...
def eval_func(algo, denoiser, env, writer, attack_type, attack_epsilon, params):
    ...
    if multiprocessing:
        ...
        unorm_score_attack = eval_multiprocess_wrapper(algo, denoiser, eval_env_under_attack_v1, env_list, _args)
        ...
        func_args = (0, algo, denoiser, env, ENV_SEED, _args)  # algo, env, start_seed, args
...
def main(args):
    ...
    if args.whitebox:
        args.eval_logdir = f"./evaluation/evaluation_online/adversarial_wb/{args.attack_type_list[0]}/attack_eps{args.attack_epsilon}/{ENV_NAME_MAPPING[args.dataset]}"
    else:
        args.eval_logdir = f"./evaluation/evaluation_online/adversarial_bb/{args.attack_type_list[0]}/attack_eps{args.attack_epsilon}/{ENV_NAME_MAPPING[args.dataset]}"
    ...
    algorithm = d3rlpy.algos.SAC(use_gpu=args.gpu,
                                 ...
                                 use_input_transformation=args.use_input_transformation,
    ...
    use_aed = not args.use_vaed
    if use_aed:
        print("Use AE Denoiser.")
        denoiser = AEDenoiser(input_dim=env.observation_space.shape[0])
    else:
        print("Use VAE Denoiser.")
        denoiser = VAEDenoiser(input_dim=env.observation_space.shape[0])
    ...
    for c, checkpoint in enumerate(list_checkpoints[:n_seeds]):
        ...
        exp_name = checkpoint.split('/')[-2]
        denoiser_ckpt = os.path.join(args.denoiser_path, exp_name, "denoiser_last.pt")
        assert os.path.isfile(denoiser_ckpt)
        denoiser_state_dict = torch.load(denoiser_ckpt)
        denoiser.load_state_dict(denoiser_state_dict['model'])
        print(f"Loaded denoiser: {denoiser_ckpt}")
        denoiser.eval()
        ...
        for n, attack_type in enumerate(args.attack_type_list):
            ...
            _unorm_score, _norm_score, _unorm_score_attack, _norm_score_attack = eval_func(algorithm, denoiser, env, writer, attack_type, args.attack_epsilon, args)
    ...
    for n in range(N):
        writer.print(
            ...
            (args.attack_type_list[n], args.attack_epsilon,
             MEAN([unorm_score_attack[n]]), np.std(unorm_score_attack, axis=1)[n],
             MEDIAN([unorm_score_attack[n]]), IQM([unorm_score_attack[n]]), OG([unorm_score_attack[n]]),
             n_seeds)
        )
        data = [[
